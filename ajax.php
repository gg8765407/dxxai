...
case 'cut_create':
	if(!$conf['cutshop_open'])exit('{"code":-1,"msg":"未开启该功能"}');
	if(!$islogin2)exit('{"code":4,"msg":"未登录"}');
	$kid = intval($_POST['kid']);
	$hashsalt=isset($_POST['hashsalt'])?$_POST['hashsalt']:null;
	$cutshop=$DB->getRow("SELECT * FROM pre_cutshop WHERE id='$kid' LIMIT 1");
	if(!$cutshop || $cutshop['active']==0){
		exit('{"code":-2,"msg":"该砍价商品不存在"}');
	}
	$tid = $cutshop['tid'];
	...
	$num = 1;
	if($cutshop['num']>1)$num = $cutshop['num'];
	$price=round($price*$num, 2);
	$cutrow = $DB->getRow("SELECT * FROM `pre_cut` WHERE `zid`='{$userrow['zid']}' AND `tid`='$tid' AND `status`='0' AND `endtime`>='$date' LIMIT 1");
	if ($cutrow)
	{
		$code = 2;
		$url = $siteurl . '?k=' .$cutrow['key'];
	} else {
		...
		$cut_row = array();
		$cut_row['price'] = $price;
		$cut_row['cut_price'] = $cutrow['cut_price'];
		$cut_row['cut_min'] = $cutshop['cut_min'];
		$cut_row['cut_max'] = $cutshop['cut_max'];
		$cut_row['pit_pattern'] = $cutshop['pit_pattern'];
		$cut_row['pit_money'] = $cutshop['pit_money'];
		$data = getCutMoney($cut_row);
		if($data['code'] == 0){
		...
		$time = $date+60*60*24*1;
		$endtime = date("Y-m-d H:i:s", $time);
		$status = $cut_money>=$price?1:0;
		if($DB->exec("INSERT INTO `pre_cut` (`kid`,`tid`,`zid`,`key`,`ip`,`price`,`date`,`endtime`,`status`) VALUES (:kid,:tid,:zid,:key,:ip,:price,NOW(),:endtime,:status)", [':kid'=>$kid, ':tid'=>$tid, ':zid'=>$userrow['zid'], ':key'=>$key, ':ip'=>$clientip, ':price'=>$price, ':endtime'=>$endtime, ':status'=>$status])){
			unset($_SESSION['addsalt']);
			$id = $DB->lastInsertId();
			$DB->exec("INSERT INTO `pre_cutlog`(`iid`,`money`,`date`,`ip`,`status`) VALUES (:iid, :money, NOW(), :ip, 1)", [':iid'=>$id, ':money'=>$cut_money, ':ip'=>$clientip]);
			$url = $siteurl . '?k=' . $key ;
		}else{
	...
	$content = str_replace('[name]',$tool['name'],$conf['cutshop_content']);
...
case 'cut_buy':
	...
	$hashsalt=isset($_POST['hashsalt'])?$_POST['hashsalt']:null;
	$cutrow = $DB->getRow("SELECT * FROM `pre_cut` WHERE `id`='$cut_id' AND `tid`='$tid' LIMIT 1");
	if($cutrow['status']<1)exit('{"code":-1,"msg":"砍价未完成！"}');
	if($cutrow['status']>=2)exit('{"code":-1,"msg":"砍价奖励已经被领取过了！"}');
	if($userrow['zid']!=$cutrow['zid'])exit('{"code":-1,"msg":"不能领取别人的砍价奖励！"}');
	$cutshop=$DB->getRow("SELECT * FROM pre_cutshop WHERE id='$kid' LIMIT 1");
	if(!$cutshop || $cutshop['active']==0){
		exit('{"code":-2,"msg":"该砍价商品不存在"}');
	}
	...
	$data = [':trade_no'=>$trade_no, ':tid'=>$tid, ':zid'=>$siterow['zid']?$siterow['zid']:1, ':type'=>'free', ':input'=>$input, ':num'=>$num, ':name'=>$tool['name'], ':money'=>$money, ':ip'=>$clientip, ':userid'=>$cookiesid, ':blockdj'=>$blockdj?$blockdj:0];
	if($DB->exec($sql, $data)){
		unset($_SESSION['addsalt']);
		$DB->exec("UPDATE `pre_cut` SET `input`='$input',`status`='2' WHERE `id`=:id", [':id'=>$cutrow['id']]);
		$srow['tid']=$tid;
		$srow['input']=$input;
		$srow['num']=$num;
		...
		if($orderid=processOrder($srow)){
			exit('{"code":0,"msg":"领取成功！你可以在进度查询中查看订单进度","orderid":"'.$orderid.'"}');
		}else{
...
case 'cut_verify':
	...
	$cut_row = $DB->getRow("SELECT * FROM `pre_cut` WHERE `key` = :key LIMIT 1", [':key'=>$key]);
	if($cut_row && $cut_row['status']==0 && $cut_row['endtime']>=$date){
		$cutshop_row = $DB->getRow("SELECT * FROM `pre_cutshop` WHERE `id`=:id LIMIT 1", [':id'=>$cut_row['kid']]);
		$cutrow = array();
		$cutrow['price'] = $cut_row['price'];
		$cutrow['cut_price'] = $cut_row['cut_price'];
		$cutrow['cut_min'] = $cutshop_row['cut_min'];
		$cutrow['cut_max'] = $cutshop_row['cut_max'];
		$cutrow['pit_pattern'] = $cutshop_row['pit_pattern'];
		$cutrow['pit_money'] = $cutshop_row['pit_money'];
		$data = getCutMoney($cutrow);
		if($data['code']!=0)exit(json_encode($data));
		if($data['code'] == 0){
			$cut_money = $data['cut_money'];
		}else{
			$cut_money = 0.01;
		}
		$cut_price = round($cut_row['cut_price'] + $cut_money, 2);
		if($cutshop_row && $cutshop_row['active']==1){
			if(!$DB->getRow("SELECT * FROM `pre_cutlog` WHERE `iid`=:iid AND `ip`=:ip LIMIT 1", [':iid'=>$cut_row['id'], ':ip'=>$clientip])){
				$DB->exec("INSERT INTO `pre_cutlog`(`iid`,`money`,`date`,`ip`,`status`) VALUES (:iid, :money, NOW(), :ip, 1)", [':iid'=>$cut_row['id'], ':money'=>$cut_money, ':ip'=>$clientip]);
				$DB->exec("UPDATE `pre_cut` SET `cut_price`='".$cut_price."' WHERE `id`=:id", [':id'=>$cut_row['id']]);
				if($cut_price>=$cut_row['price']){
					$DB->exec("UPDATE `pre_cut` SET `status`='1' WHERE `id`=:id", [':id'=>$cut_row['id']]);
				}
				$result = array('code' => 0, 'msg' => '恭喜你，帮他砍掉 '.$cut_money.' 元', 'key'=>$key);
			}else{
				$result = array('code' => -1, 'msg' => '你已帮他砍过了哦！', 'key'=>$key);
			}
		}else{
			$result = array('code' => -1, 'msg' => '该砍价商品不存在！', 'key'=>$key);
		}
	}else{
		$result = array('code' => -1, 'msg' => '该砍价任务不存在！', 'key'=>$key);
	}
	exit(json_encode($result));
	break;
case 'cut_content':
	...
	$cutrow = $DB->getRow("SELECT * FROM `pre_cut` WHERE `id`='$id' LIMIT 1");
	$tool=$DB->getRow("SELECT * FROM pre_tools WHERE tid='$tid' LIMIT 1");

	if ($cutrow) {
		if($cutrow['status']>=1)exit('{"code":-1,"msg":"该团购小队已经完成，奖励已经到账，请重新创建团购小队进行组团！"}');
		if($cutrow['endtime']<$date)exit('{"code":-1,"msg":"该团购小队已经过期，请重新创建团购小队进行组团！"}');
		$url = $siteurl . '?k=' . $cutrow['key'];
	...
	break;
case 'group_create':
	if(!$conf['groupshop_open'])exit('{"code":-1,"msg":"未开启该功能"}');
	if(!$islogin2)exit('{"code":4,"msg":"未登录"}');
	...
	$groupshop=$DB->getRow("SELECT * FROM pre_groupshop WHERE id='$gid' LIMIT 1");
	if(!$groupshop || $groupshop['active']==0){
		exit('{"code":-2,"msg":"该团购商品不存在"}');
	}
	$tid = $groupshop['tid'];
	$tool=$DB->getRow("SELECT * FROM pre_tools WHERE tid='$tid' LIMIT 1");
	if(!$tool || $tool['active']==0){
	...
	if($groupshop['nummax']>0 && $num>$groupshop['nummax'])exit('{"code":-1,"msg":"当前商品最大下单数量为'.$groupshop['nummax'].'"}');
	if(isset($price_obj)){
		$price_obj->setToolInfo($tid,$tool);
		$price=$price_obj->getToolPrice($tid);
	...
	$price=round($price-$groupshop['money'], 2);
	$money=round($price*$num, 2);
	if($userrow['rmb']<$money)exit('{"code":-1,"msg":"帐户余额不足，请充值！"}');
	$input=$inputvalue.($inputvalue2?'|'.$inputvalue2:null).($inputvalue3?'|'.$inputvalue3:null).($inputvalue4?'|'.$inputvalue4:null).($inputvalue5?'|'.$inputvalue5:null);

	$grouprow = $DB->getRow("SELECT * FROM `pre_group` WHERE `zid`='{$userrow['zid']}' AND `tid`='$tid' AND `status`='0' AND `endtime`>='$date' LIMIT 1");
	if ($grouprow)
	{
		if($grouprow['input']!=$input){
			$DB->exec("UPDATE `pre_group` SET `input`=:input WHERE `id`=:id", [':input'=>$input, ':id'=>$grouprow['id']]);
		}
		$code = 2;
		$url = $siteurl . '?g=' .$grouprow['key'];
	} else {
		if($conf['verify_open']==1 && (empty($_SESSION['addsalt']) || $hashsalt!=$_SESSION['addsalt'])){
			exit('{"code":-1,"msg":"验证失败，请刷新页面重试"}');
		}
		...
		$time = $date+60*60*24*1;
		$endtime = date("Y-m-d H:i:s", $time);
		if($DB->exec("INSERT INTO `pre_group` (`gid`,`tid`,`zid`,`input`,`key`,`ip`,`plan`,`click`,`date`,`endtime`,`status`) VALUES (:gid,:tid,:zid,:input,:key,:ip,:plan,1,NOW(),:endtime,0)", [':gid'=>$gid, ':tid'=>$tid, ':zid'=>$userrow['zid'], ':input'=>$input, ':key'=>$key, ':ip'=>$clientip, ':plan'=>$groupshop['people'], ':endtime'=>$endtime])){
			$iid = $DB->lastInsertId();
			unset($_SESSION['addsalt']);
			if($DB->exec("INSERT INTO `pre_grouplog`(`iid`,`groupzid`,`zid`,`money`,`num`,`input`,`date`,`ip`,`status`) VALUES (:iid, :groupzid, :zid, :money, :num, :input, NOW(), :ip, 0)", [':iid'=>$iid, ':groupzid'=>$userrow['zid'], ':zid'=>$userrow['zid'], ':money'=>$money, ':num'=>$num, ':input'=>$input, ':ip'=>$clientip])){
				$trade_no='group'.date("YmdHis").rand(111,999);
				...
				$data = [':trade_no'=>$trade_no, ':tid'=>$tid, ':zid'=>$siterow['zid']?$siterow['zid']:1, ':type'=>'rmb', ':input'=>$input, ':num'=>$num, ':name'=>$tool['name'], ':money'=>$money, ':ip'=>$clientip, ':userid'=>$cookiesid];
				if($DB->exec($sql, $data)){
					changeUserMoney($userrow['zid'], $money, false, '扣除', '你发起了组团下单商品【'.$tool['name'].'】的团购小队，扣除'.$money.'元余额');
				}
			}
			$url = $siteurl . '?g=' . $key ;
		}else{
	...
	if($conf['fanghong_api']>0)$url = fanghongdwz($url);
	$content = str_replace('[name]',$tool['name'],$conf['groupshop_content']);
	$content = str_replace('[url]',$url,$content);
	if(!$content)$content = $url;
	$result = array('code'=>0, 'msg'=>'succ', 'url'=>$url, 'content'=>$content);
	exit(json_encode($result));
...
case 'group_verify':
	...
	if(!$grouprow){
		exit('{"code":-2,"msg":"该团购小队不存在"}');
	}elseif($grouprow['status']>=1){
		exit('{"code":-2,"msg":"该团购小队已完成"}');
	}elseif($grouprow['endtime']<$date){
		exit('{"code":-2,"msg":"该团购小队已过期"}');
	}
	$grouplogrow = $DB->getRow("SELECT * FROM `pre_grouplog` WHERE `iid`=:iid AND `zid`=:zid LIMIT 1", [':iid'=>$grouprow['id'], ':zid'=>$userrow['zid']]);
	if($grouplogrow){
		exit('{"code":-2,"msg":"你已加入该团购小队"}');
	}
	$groupshop = $DB->getRow("SELECT * FROM `pre_groupshop` WHERE `id`=:id LIMIT 1", [':id'=>$grouprow['gid']]);
	if(!$groupshop || $groupshop['active']==0){
		exit('{"code":-2,"msg":"该团购商品不存在"}');
	}
	$tid = $groupshop['tid'];
	...
	$money=round($price*$num, 2);
	$input=$inputvalue.($inputvalue2?'|'.$inputvalue2:null).($inputvalue3?'|'.$inputvalue3:null).($inputvalue4?'|'.$inputvalue4:null).($inputvalue5?'|'.$inputvalue5:null);
	$click=$grouprow['click']+1;

	if($userrow['rmb']<$money)exit('{"code":-1,"msg":"帐户余额不足，请充值！"}');
	if($DB->exec("INSERT INTO `pre_grouplog`(`iid`,`groupzid`,`zid`,`money`,`num`,`input`,`date`,`ip`,`status`) VALUES (:iid, :groupzid, :zid, :money, :num, :input, NOW(), :ip, 0)", [':iid'=>$grouprow['id'], ':groupzid'=>$grouprow['zid'], ':zid'=>$userrow['zid'], ':money'=>$money, ':num'=>$num, ':input'=>$input, ':ip'=>$clientip])){
		$trade_no='group'.date("YmdHis").rand(111,999);
		$sql="INSERT INTO `pre_pay` (`trade_no`,`tid`,`zid`,`type`,`input`,`num`,`name`,`money`,`ip`,`userid`,`addtime`,`blockdj`,`status`) VALUES (:trade_no, :tid, :zid, :type, :input, :num, :name, :money, :ip, :userid, NOW(), 0, 1)";
		$data = [':trade_no'=>$trade_no, ':tid'=>$tid, ':zid'=>$siterow['zid']?$siterow['zid']:1, ':type'=>'rmb', ':input'=>$input, ':num'=>$num, ':name'=>$tool['name'], ':money'=>$money, ':ip'=>$clientip, ':userid'=>$cookiesid];
		if($DB->exec($sql, $data)){
			if($money>0)changeUserMoney($userrow['zid'], $money, false, '扣除', '你加入【ID：'.$grouprow['id'].'】的团购小队，扣除'.$money.'元余额');
			$DB->exec("UPDATE `pre_group` SET `click`=`click`+1 WHERE `id`=:id", [':id'=>$grouprow['id']]);
			if($click>=$grouprow['plan']){
				if($DB->exec("UPDATE `pre_group` SET `status`='1' WHERE `id`=:id", [':id'=>$grouprow['id']])){
					$zid=$siterow['zid']?$siterow['zid']:1;
					do_group_goods($zid, $grouprow, $trade_no);
				}
			}
			$result = array('code' => 0, 'msg' => '恭喜你，成功加入该团购小队！', 'key'=>$key);
		}else{
			$result = array('code' => -1, 'msg' => '加入团购小队失败，请重试！', 'key'=>$key);
		}
	}else{
		$result = array('code' => -1, 'msg' => '加入团购小队失败，请重试！', 'key'=>$key);
	}
...
case 'group_content':
	...
	$id = intval($_POST['id']);
	$tid = intval($_POST['tid']);
	$grouprow = $DB->getRow("SELECT * FROM `pre_group` WHERE `id`='$id' LIMIT 1");
	$tool=$DB->getRow("SELECT * FROM pre_tools WHERE tid='$tid' LIMIT 1");

	if ($grouprow) {
		if($grouprow['status']>=1)exit('{"code":-1,"msg":"该团购小队已经完成，已下单，请重新创建团购小队进行组团！"}');
		if($grouprow['endtime']<$date)exit('{"code":-1,"msg":"该团购小队已经过期，请重新创建团购小队进行组团！"}');
		$url = $siteurl . '?g=' . $grouprow['key'];
...
case 'group_query':
	...
	$re = $DB->query("SELECT A.*,B.`name` FROM `pre_group` A LEFT JOIN `pre_tools` B ON A.`tid`=B.`tid` WHERE A.`zid`='$zid' ORDER BY A.`id` DESC LIMIT 30");
	$ar_log = [];
	while ($res = $re->fetch()) {
		$ar_log[] = [
			'id' => $res['id'],
	...
	if (count($ar_log) == 0) exit(json_encode(['code' => -1, 'msg' => '无相关数据,请先去生成对应的团购链接再来查询！']));
...
case 'seckill_create':
	...
	if($seckillshop['starttime']>$date){
		exit('{"code":-2,"msg":"该秒杀商品活动还未开始，秒杀开始时间:'.$seckillshop['starttime'].'"}');
	}
	if($seckillshop['endtime']<=$date){
...
case 'seckill_buy':
	...
	if($DB->exec($sql, $data)){
		...
		if($money>0)changeUserMoney($userrow['zid'], $money, false, '扣除', '你参加秒杀活动下单了商品【'.$tool['name'].'】，扣除'.$money.'元余额');
...
case 'payrmb':
	...
	if($srow['status']==0){
		...
		if($DB->exec("UPDATE `pre_site` SET `rmb`=`rmb`-'{$srow['money']}' WHERE `zid`='{$userrow['zid']}'") && $DB->exec("UPDATE `pre_pay` SET `type`='rmb',`status`='1',`endtime`=NOW() WHERE `trade_no`='{$orderid}'")){
			...
			if($orderid=processOrder($srow)){
				...
				/*$ros=$DB->getRow("SELECT * FROM pre_tools WHERE tid='{$srow['tid']}' LIMIT 1");
                $money1=round($conf['suppic']*$srow['num'], 2);
                $DB->exec("UPDATE `pre_site` SET `rmbtc`=`rmbtc`+'{$money1}' WHERE `zid`='{$ros['zid']}'");
		$DB->exec("INSERT INTO `pre_points` (`zid`, `action`, `point`, `bz`, `addtime`, `orderid`, `status`) VALUES (:zid, :action, :point, :bz, NOW(), :orderid, :status)", [':zid'=>$ros['pro'], ':action'=>'提成', ':point'=>$money1, ':bz'=>'你的商品被其他用户购买', ':orderid'=>$orderid, ':status'=>'0']);*/
	...
	exit(json_encode($result));
	    	break;
	    		case 'cidrs2':
	    		    	$cid=intval($_GET['cid']);
	    		    $rs=$DB->query("SELECT * FROM pre_class WHERE cid='$cid'");
	    		    $res = $rs->fetch();
	    		    $cy=strlen($res['nr']);
	    		    if($cy>90){$cy=1;}else{$cy=0;}
	    		    $result=array("nr"=>$res['nr'],"cid"=>$cid,"num"=>$cy);
	    		     	exit(json_encode($result));
	    		    	break;
	    		case 'cidrs':
	    		    	$cid=intval($_GET['cid']);
...
case 'gettool':
	...
	while($res = $rs->fetch(PDO::FETCH_ASSOC)){
		...
		$invitegift_money = 0;
		$invite_gift = 0;
		if($invite_row=$DB->getRow("SELECT * FROM pre_invitegiftshop WHERE tid='{$res['tid']}' LIMIT 1")){
			$isinvitegift = 1;
			$invitegift_money = $invite_row['money'];
			$invite_gift = $invite_row['gift'];
		}
		$data[]=array('tid'=>$res['tid'],'cid'=>$res['cid'],'sort'=>$res['sort'],'name'=>$res['name'],'value'=>$res['value'],'price'=>$price,'input'=>$res['input'],'inputs'=>$res['inputs'],'desc'=>$res['desc'],'alert'=>$res['alert'],'shopimg'=>$res['shopimg'],'repeat'=>$res['repeat'],'multi'=>$res['multi'],'close'=>$res['close'],'prices'=>$res['prices'],'min'=>$res['min'],'max'=>$res['max'],'sales'=>$res['sales'],'isfaka'=>$isfaka,'stock'=>$res['stock'],'isinvitegift'=>$isinvitegift,'invitegift_money'=>$invitegift_money,'invite_gift'=>$invite_gift);
	}
